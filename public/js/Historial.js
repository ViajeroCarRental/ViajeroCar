document.addEventListener("DOMContentLoaded", () => {
  ...
  function loadData() {
    ...
    // ✅ YA SE ENVIAN TODOS LOS FILTROS
    const params = new URLSearchParams({
      ...
      fpago: fpago.value || "",
      fsucursal: fsucursal.value || "",
      fvehiculo: fvehiculo.value || "",
    ...
    fetch(`/api/historial?${params}`)
      ...
      .then(json => {
        ...
        const start = json.total === 0 ? 0 : (page - 1) * Number(pp.value) + 1;
        const end = json.total === 0 ? 0 : start + json.data.length - 1;
        ...
        if (!json.data.length) {
          tbody.innerHTML = `
            <tr>
              <td colspan="11" style="text-align:center;color:#667085">Sin resultados</td>
            </tr>
          `;
          return;
        }
      ...
      .catch(() => {
        tbody.innerHTML = `
          <tr>
            <td colspan="11" style="text-align:center;color:#B42318">Error al cargar historial</td>
          </tr>
        `;
      });
  }
