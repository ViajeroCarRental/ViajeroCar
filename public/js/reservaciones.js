/* ==========================================================
   ...
   Extraído del Visor de reservación y hecho reutilizable.

   API:
     OverlayActualizacion.cargando();          // coche + mensajes rotando
     OverlayActualizacion.exito();             // manita + "¡Información actualizada!"
     OverlayActualizacion.ocultar();
     OverlayActualizacion.config                // mensajes, tiempos, etc.

   Enganche automático a formularios:
     <form data-overlay-actualizacion> ... </form>
   Muestra el overlay, espera un momento y envía el formulario.
   Al recargar, si existe .alert-success (configurable), muestra
   el estado de éxito.
========================================================== */
(function (global) {
    'use strict';
    ...
    const CONFIG = {
        ...
        intervaloMensajes: 1500,   // cada cuánto rota el texto
        retrasoEnvio: 800,         // espera antes de enviar el formulario
        tiempoMaximo: 12000,       // seguro por si el envío se queda colgado
        duracionExito: 1200,       // cuánto se ve la manita
    ...
    function conectar() {
        ...
        // Al terminar, un clic cierra la tarjeta de éxito
        overlay.addEventListener('click', () => {
            if (overlay.classList.contains('is-success')) ocultar();
        });

        // Formularios marcados con data-overlay-actualizacion
        ...
        // Al volver de guardar: si el servidor mandó éxito, se muestra la manita
...
window.addEventListener('load', function () {
    ...
    if (!form || typeof OverlayActualizacion === 'undefined') return;

    form.addEventListener('submit', function (e) {
        if (e.defaultPrevented) return;   // faltan campos: no se muestra nada
