const qs = s=>document.querySelector(s);
  ...
  function renderBirthCal(){
    ...
    birthPop.innerHTML = `
      <div class="cal-head">
        <button type="button" class="cal-nav-btn" aria-label="Año anterior" data-nav="-12"><i class="fa-solid fa-angles-left"></i></button>
        ...
        <div class="cal-controls">
          ...
          <select class="cal-year" aria-label="Año">
        ...
        <button type="button" class="cal-nav-btn" aria-label="Año siguiente" data-nav="12"><i class="fa-solid fa-angles-right"></i></button>
      </div>
      <div class="cal-grid">
    ...
    birthPop.querySelectorAll('[data-nav]').forEach(btn=>{
      btn.addEventListener('click', ()=>{
        ...
        // clamp año a rango permitido para DOB (no futuro)
  ...
  function seedReservations(){
    ...
    const sample = [
      { id:'CT-' + (10000+Math.floor(Math.random()*89999)), car:'Chevrolet Aveo o similar', dates:`${fmt(addDays(2))} → ${fmt(addDays(5))}`, status:'pendiente' },
      { id:'CT-' + (10000+Math.floor(Math.random()*89999)), car:'Kia Sportage o similar',    dates:`${fmt(addDays(-1))} → ${fmt(addDays(3))}`, status:'activa' },
      { id:'CT-' + (10000+Math.floor(Math.random()*89999)), car:'Volkswagen Virtus o similar',dates:`${fmt(addDays(-14))} → ${fmt(addDays(-10))}`, status:'finalizada' },
      { id:'CT-' + (10000+Math.floor(Math.random()*89999)), car:'BMW Serie 3 o similar',     dates:`${fmt(addDays(12))} → ${fmt(addDays(16))}`, status:'pendiente' },
    ];
    ...
    list.forEach(r=>{
      ...
      tr.innerHTML = `
        ...
        <td><span class="status ${cls(r.status)}">
          ${r.status==='pendiente'?'🕒 Pendiente': r.status==='activa'?'✅ Activa':'✔ Finalizada'}
        </span></td>`;
  ...
  function seedCards(){
    const cards = [
      ...
      { id:crypto.randomUUID?.()||String(Date.now()+1), brand:'mastercard', number:'5555555555554444', name:'Juan Pérez', exp:'03/27', alias:'Empresa' },
    ];
  ...
  cardForm.addEventListener('submit', e=>{
    ...
    if(rawNum.length < 15){ alert('Número de tarjeta inválido.'); return; }
    const data = {
