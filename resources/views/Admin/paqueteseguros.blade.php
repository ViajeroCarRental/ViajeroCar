...
<style>
    ...
    .modal-content {
        width: 95% !important;
        max-width: 900px !important; /* Más ancho para las 2 columnas principales */
        padding: 20px 30px;
    }
    ...
    .modal-layout {
        ...
        gap: 30px; /* Separación entre la columna izquierda y derecha */
    }

    /* 4. Grid interno para la columna izquierda (Campos) */
    .form-grid {
        display: grid;
        grid-template-columns: 1fr; /* Una sola columna dentro de la parte izquierda */
        gap: 15px;
    }
    ...
    .contenedor-porcentajes, .contenedor-protecciones {
        display: grid;
        grid-template-columns: 1fr; /* Una columna para las listas, puedes cambiarlo a 1fr 1fr si las quieres más apretadas */
        gap: 10px;
        max-height: 220px; /* Un poco más alto para que quepan bien */
        overflow-y: auto; 
        border: 1px solid #cbd5e1; 
        padding: 12px; 
        border-radius: 6px; 
...
<div class="modal" id="modalNuevo">
  <div class="modal-content">
    <h3 style="margin-bottom: 20px; color: #1e3a8a; border-bottom: 2px solid #e2e8f0; padding-bottom: 10px;">Nuevo Paquete de Seguros</h3>
    ...
    <div class="modal-layout">
        <div class="form-grid">
            ...
            <div style="display: flex; gap: 10px;">
                <div style="flex: 1;">
                    ...
                    <input type="number" step="0.01" id="newDeducibleColision" value="0.00">
                </div>
                <div style="flex: 1;">
                    <label>Deducible Robo ($)</label>
                    <input type="number" step="0.01" id="newDeducibleRobo" value="0.00">
                </div>
            </div>
        ...
        <div>
            ...
            <label style="font-weight: bold; color: #1e3a8a; display: block; margin-bottom: 5px;">% de Garantía por Categoría</label>
            <div class="contenedor-porcentajes">
                @foreach($categorias as $cat)
                    <div class="fila-porcentaje">
                        <span>🚗 <strong>{{ $cat->nombre }}</strong> (Base: ${{ number_format($cat->garantia_base, 0) }})</span>
                        <div style="display: flex; align-items: center; gap: 5px;">
                            <input type="number" class="input-pct new-porcentaje" data-id="{{ $cat->id_categoria }}" min="0" max="100" value="0"> %
                        </div>
                    </div>
                @endforeach
            </div>
            ...
            <label style="font-weight: bold; color: #1e3a8a; display: block; margin-bottom: 5px;">Protecciones Incluidas</label>
    ...
    </div> <div class="modal-footer" style="margin-top:20px; border-top: 1px solid #e2e8f0; padding-top: 15px; text-align: right;">
...
<div class="modal" id="modalEditar">
  <div class="modal-content">
    <h3 style="margin-bottom: 20px; color: #1e3a8a; border-bottom: 2px solid #e2e8f0; padding-bottom: 10px;">Editar Paquete</h3>
    ...
    <div class="modal-layout">
        <div class="form-grid">
            ...
            <div style="display: flex; gap: 10px;">
                <div style="flex: 1;">
                    <label>Deducible Colisión ($)</label>
                    <input type="number" step="0.01" id="editDeducibleColision">
                </div>
                <div style="flex: 1;">
                    <label>Deducible Robo ($)</label>
                    <input type="number" step="0.01" id="editDeducibleRobo">
                </div>
            </div>
            ...
            <div class="contenedor-porcentajes">
                @foreach($categorias as $cat)
                    <div class="fila-porcentaje">
                        ...
                        <div style="display: flex; align-items: center; gap: 5px;">
                            <input type="number" class="input-pct edit-porcentaje" data-id="{{ $cat->id_categoria }}" min="0" max="100"> %
                        </div>
                    </div>
                @endforeach
            </div>
