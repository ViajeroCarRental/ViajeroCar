      ...
      @forelse ($reservaciones as $r)
        @php
            ...
            $fmtFecha = function ($fecha) {
                ...
                return str_replace('.', '', strtolower(\Carbon\Carbon::parse($fecha)->translatedFormat('d-M-Y')));
            };

            $fmtHora = function ($hora) {
                return $hora ? \Carbon\Carbon::parse($hora)->format('H:i') : '—';
            };
        @endphp
            ...
            <div class="tbody">
                @forelse ($reservaciones as $r)
                    @php
                        ...
                        if ($nombreCompleto === '') {
                            $nombreCompleto = trim(
                                (string) ($r->nombre_cliente ?? '') . ' ' . (string) ($r->apellidos_cliente ?? ''),
                            );
                        }
                    ...
                    <div class="row" data-codigo="{{ $r->codigo }}" data-cliente="{{ $nombreCompleto }}"
                        data-email="{{ $r->email_cliente }}" data-numero="{{ $r->telefono_cliente }}"
                        data-categoria="{{ $r->categoria }}"
                        data-fecha-salida="{{ \Carbon\Carbon::parse($r->fecha_inicio)->format('Y-m-d') }}"
                        data-estado="{{ $r->estado }}" data-sucursal="{{ $r->sucursal_retiro }}"
                        ...
                        <div class="reserva-summary">
                            ...
                            <div class="summary-actions">
                                ...
                                <div class="summary-actions-left">
                                    ...
                                    <button type="button" class="btn btn-cancel" title="Cancelar reservación"
                                        data-open-actions data-id="{{ $r->id_reservacion }}"
                                ...
                                <div class="summary-actions-right">
                                    ...
                                    <button type="button" class="btn btn-mail"
                                        onclick="reenviarCorreo({{ $r->id_reservacion }}, this)">
                                        <i class="fa-solid fa-envelope"></i>
                                        Reenviar correo
                                    </button>

                                    <button type="button" class="btn btn-car btn-apartar-auto"
                                        data-id="{{ $r->id_reservacion }}">
                                        <i class="fa-solid fa-car-side"></i>
                                        Apartar auto
                                    </button>
            ...
            <div class="box box-xl">
                <header>
                    <div>
                        ...
                        <span>Bookings del día anterior · Total: <b
                                id="countPrev">{{ count($reservaciones_anteriores) }}</b></span>
                    </div>
                    <button type="button" id="pClose">&times;</button>
                </header>

                <div class="cnt table-cnt">
                    <section id="tablaPrevias" class="table {{ $esAeropuerto ? 'is-airport' : '' }}"
                        data-cols="{{ $cols }}">
                        <div class="thead">
                            <div></div>
                            <div>No. de Reservacion</div>
                        ...
                        <div class="tbody">
                            @forelse ($reservaciones_anteriores as $r)
                                ...
                                <div class="row" data-id="{{ $r->id_reservacion }}"
                                    data-codigo="{{ $r->codigo }}" data-cliente="{{ $nombreCompleto }}"
                                    data-email="{{ $r->email_cliente }}" data-numero="{{ $r->telefono_cliente }}"
                                    data-categoria="{{ $r->categoria }}"
                                    data-fecha-salida="{{ \Carbon\Carbon::parse($r->fecha_inicio)->format('Y-m-d') }}"
                                    data-estado="{{ $r->estado }}" data-sucursal="{{ $r->sucursal_retiro }}">
                                    <div class="actions-wrap">
                                        <button type="button" class="iconbtn more" title="Más acciones"
                                            data-open-actions data-id="{{ $r->id_reservacion }}"
                                            data-codigo="{{ $r->codigo }}"
                                            data-delete-url="{{ route('rutaEliminarReservacionActiva', $r->id_reservacion) }}">
                                            ⋯
                            ...
                            @empty
                                <div class="row">
                                    <div style="grid-column: 1 / -1; text-align:center;">No hay reservaciones anteriores.
                                    </div>
                                </div>
                            @endforelse
                        </div>
