...
<main class="page">
    <section class="hero hero-mini">
        <div class="hero-bg">
            <img src="{{ asset('img/politicas.webp') }}"
                 alt="{{ __('Policies') }}"
        ...
        <div class="hero-content-politicas">
            ...
            <div class="mobile-search-trigger d-block d-xl-none">
                <div class="mobile-search-box">
                    <p class="mobile-search-text">{{ __('Find your car here') }}</p>
                    <button type="button" id="btn-abrir-buscador-politicas" class="mobile-search-btn">
                        <i class="fa-solid fa-magnifying-glass" aria-hidden="true"></i> {{ __('SEARCH') }}
                    </button>
                </div>
            </div>
                            ...
                            <div class="sg-col sg-col-datetime">
                                ...
                                <div class="field">
                                    ...
                                    <div class="datetime-row">
                                        <div class="dt-field icon-field">
                                            <span class="field-icon"><i class="fa-regular fa-calendar-days" aria-hidden="true"></i></span>
                                            <input id="pickupDatePoliticas" name="pickup_date" type="text"
                                                   placeholder="{{ __('Date') }}"
                                                   value="{{ request('pickup_date') }}"
                                                   data-min="{{ now()->toDateString() }}">
                                        </div>
                                        <div class="dt-field icon-field time-field">
                                            <span class="field-icon"><i class="fa-regular fa-clock" aria-hidden="true"></i></span>
                                            <input type="text" id="pickupTimePoliticas" name="pickup_time"
                                                   placeholder="{{ __('Time') }}"
                                ...
                                <div class="field">
                                    ...
                                    <div class="datetime-row">
                                        <div class="dt-field icon-field">
                                            <span class="field-icon"><i class="fa-regular fa-calendar-days" aria-hidden="true"></i></span>
                                            <input id="dropoffDatePoliticas" name="dropoff_date" type="text"
                                                   placeholder="{{ __('Date') }}"
                                        ...
                                        <div class="dt-field icon-field time-field">
                                            <span class="field-icon"><i class="fa-regular fa-clock" aria-hidden="true"></i></span>
                                            <input type="text" id="dropoffTimePoliticas" name="dropoff_time"
                                                   placeholder="{{ __('Time') }}"
    ...
    {{-- Overlay del Flatpickr (antes era creado por JS) --}}
    <div id="fp-overlay" class="fp-overlay" aria-hidden="true"></div>
    ...
    <section class="policies-wrap">
        <div class="policies-grid">
            ...
            <button type="button" class="policy-card" data-modal="tpl-requisitos" data-title="{{ __('Rental requirements') }}">
                <span class="pc-icon"><i class="fa-solid fa-clipboard-check" aria-hidden="true"></i></span>
            ...
            <button type="button" class="policy-card" data-modal="tpl-pago" data-title="{{ __('Payment methods') }}">
                <span class="pc-icon"><i class="fa-solid fa-wallet" aria-hidden="true"></i></span>
            ...
            <button type="button" class="policy-card" data-modal="tpl-depositos" data-title="{{ __('Security deposits') }}">
                <span class="pc-icon"><i class="fa-solid fa-mobile-screen-button" aria-hidden="true"></i></span>
            ...
            <button type="button" class="policy-card" data-modal="tpl-precheckin" data-title="{{ __('Pre Check-In') }}">
                <span class="pc-icon"><i class="fa-solid fa-circle-check" aria-hidden="true"></i></span>
            ...
            <button type="button" class="policy-card" data-modal="tpl-cancelaciones" data-title="{{ __('Cancellations and refunds') }}">
                <span class="pc-icon"><i class="fa-solid fa-rotate-left" aria-hidden="true"></i></span>
            ...
            <button type="button" class="policy-card" data-modal="tpl-seguro-cancelacion" data-title="{{ __('Cancellation insurance') }}">
                <span class="pc-icon"><i class="fa-solid fa-shield-heart" aria-hidden="true"></i></span>
            ...
            <button type="button" class="policy-card" data-modal="tpl-limpieza" data-title="{{ __('Cleaning policy') }}">
                <span class="pc-icon"><i class="fa-solid fa-spray-can-sparkles" aria-hidden="true"></i></span>
            ...
            <button type="button" class="policy-card" data-modal="tpl-infracciones" data-title="{{ __('Infractions policy') }}">
                <span class="pc-icon"><i class="fa-solid fa-file-lines" aria-hidden="true"></i></span>
            ...
            <button type="button" class="policy-card" data-modal="tpl-privacidad" data-title="{{ __('Privacy notice') }}">
                <span class="pc-icon"><i class="fa-solid fa-shield-halved" aria-hidden="true"></i></span>
            ...
            <button type="button" class="policy-card" data-modal="tpl-terminos" data-title="{{ __('Terms and conditions') }}">
                <span class="pc-icon"><i class="fa-solid fa-scale-balanced" aria-hidden="true"></i></span>
            ...
            <button type="button" class="policy-card" data-modal="tpl-cargos" data-title="{{ __('Charges and taxes definitions') }}">
                <span class="pc-icon"><i class="fa-solid fa-receipt" aria-hidden="true"></i></span>
    ...
    <template id="tpl-requisitos">
        <div class="policy-content">
            ...
            <p>{{ __('The primary customer must present a valid driver\'s license from their country of residence with a minimum of one year of seniority. Viajero Car Rental reserves the right to verify the license and deny the rental if it does not meet the guidelines. For additional drivers, only a valid license is required, without a minimum seniority.') }}</p>

            <h4>{{ __('Valid official ID') }}</h4>
            <p>{{ __('A valid official photo ID must be presented. For Mexican citizens, an INE/IFE with at least one year of seniority is required. For foreigners, a passport, Passport Card, FM2, FM3 or Permanent Residence is accepted.') }}</p>

            <h4>{{ __('Credit card in the renter\'s name') }}</h4>
            <p>{{ __('It is essential to present a bank credit card in the renter\'s name to start the rental. The final payment can be made with a debit card. American Express, Visa and MasterCard are accepted. The card must have a minimum of one year of seniority. Viajero Car Rental may reject damaged, illegible cards or special program cards that are not compatible.') }}</p>

            <h4>{{ __('Right to deny service') }}</h4>
            <p>{{ __('Viajero Car Rental reserves the right not to complete the rental if any of the requirements are not met or if any irregularity is detected in the documentation presented.') }}</p>
        </div>
    </template>
    ...
    <template id="tpl-seguro-cancelacion">
        <div class="policy-content">
            ...
            <p>{{ __('The refund will be processed and reflected on the card used for the prepayment, within a period of up to') }} <strong>{{ __('8 business days') }}</strong>.</p>
        </div>
    </template>
    ...
    <template id="tpl-depositos">
        <div class="policy-content">
            ...
            <h5>{{ __('Release') }}</h5>
            <p>{{ __('It is generally released within') }} <strong>{{ __('48 business hours') }}</strong> {{ __('after return, or within the period indicated by the issuing bank, provided the vehicle is returned in proper condition.') }}</p>
            ...
            <p class="note-italic"><em>{{ __('Amounts subject to change without prior notice.') }}</em></p>
        </div>
    </template>
    ...
    <template id="tpl-infracciones">
        <div class="policy-content">
            ...
            <ul>
                <li><strong>{{ __('Phone:') }}</strong> <a href="tel:524427169793">(+52) 442 716 9793</a></li>
                <li><strong>{{ __('Email:') }}</strong> <a href="mailto:reservaciones@viajerocarental.com">reservaciones@viajerocarental.com</a></li>
                <li><strong>{{ __('Technical Support:') }}</strong> <a href="mailto:soportetecnico@viajerocarental.com">soportetecnico@viajerocarental.com</a></li>
            </ul>

            <p>{{ __('The customer will be responsible for all fines incurred during the rental period. These may amount up to') }} <strong>$25,000.00 MXN</strong>, {{ __('depending on the violation and local traffic regulations.') }}</p>

            <p><em>{{ __('Additionally, an administrative fee of') }} <strong>$1,000.00 MXN</strong> {{ __('(plus fees and taxes) will apply.') }}</em></p>
        </div>
    </template>
    ...
    <template id="tpl-terminos">
        <div class="policy-content">
            ...
            <p>{{ __('The approximate total rental amount is based on the information provided at the time of booking. The driver must present a valid credit card with sufficient available balance, a valid driver\'s license and an official ID when opening the contract...') }}</p>
            <p>
    ...
    foreach ($ciudadesDropoff as $ciudad) {
        foreach ($ciudad->sucursalesActivas as $suc) {
            ...
            if (str_contains($name, 'aeropuerto')) {
                $iconosPorSucursal[$suc->id_sucursal] = 'fa-plane-departure';
            } elseif (str_contains($name, 'terminal')
                   || (str_contains($name, 'central') && !str_contains($name, 'plaza central park'))) {
                $iconosPorSucursal[$suc->id_sucursal] = 'fa-bus';
            } else {
                $iconosPorSucursal[$suc->id_sucursal] = 'fa-building';
            }
        }
    }
@endphp
...
@section('js-vistaPoliticas')
    {{-- Mapa de iconos (debe declararse ANTES de politicas.js) --}}
    <script>window.iconosPorId = @json($iconosPorSucursal);</script>

    {{-- Librerías externas (defer para no bloquear el render) --}}
    <script defer src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
    <script defer src="https://cdn.jsdelivr.net/npm/select2@4.1.0-rc.0/dist/js/select2.min.js"></script>
    <script defer src="https://cdn.jsdelivr.net/npm/flatpickr"></script>
    <script defer src="https://cdn.jsdelivr.net/npm/flatpickr/dist/l10n/es.js"></script>

    {{-- Script de la vista (se ejecuta después de las librerías por defer) --}}
    <script defer src="{{ asset('js/politicas.js') }}"></script>
@endsection
