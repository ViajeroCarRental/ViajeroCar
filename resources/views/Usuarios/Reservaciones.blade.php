    ...
    <div class="field-row">
        ...
        <div class="field">
            <div class="ctl has-ico pristine" data-float>
                ...
                <select name="pickup_sucursal_id" id="pickupPlace" required data-float-select>
                    ...
                    @foreach($ciudades->where('nombre','Querétaro') as $ciudad)
                        <optgroup label="{{ $ciudad->nombre }}{{ $ciudad->estado ? ' — '.$ciudad->estado : '' }}">
                            @foreach($ciudad->sucursalesActivas ?? [] as $suc)
                                ...
                                @endphp
                                <option value="{{ $suc->id_sucursal }}" data-icon="{{ $icon }}" {{ (string)$pickupSucursalId===(string)$suc->id_sucursal ? 'selected' : '' }}>
                                    {{ $suc->nombre }}
                                </option>
                            @endforeach
                        </optgroup>
        ...
        <div class="field">
            <div class="ctl has-ico pristine" data-float>
                ...
                <select name="dropoff_sucursal_id" id="dropoffPlace" required data-float-select>
                    ...
                    @foreach(($ciudades ?? []) as $ciudad)
                        <optgroup label="{{ $ciudad->nombre }}{{ $ciudad->estado ? ' — '.$ciudad->estado : '' }}">
                            @foreach($ciudad->sucursalesActivas ?? [] as $suc)
                                @php
                                    $name = strtolower($suc->nombre);
                                    $iconDev = 'fa-solid fa-location-dot';
                                    if (str_contains($name, 'aeropuerto')) $iconDev = 'fa-solid fa-plane-departure';
                                    elseif (str_contains($name, 'central de autobuses')) $iconDev = 'fa-solid fa-bus';
                                    elseif (str_contains($name, 'oficina') || str_contains($name, 'central park')) $iconDev = 'fa-solid fa-building';
                                @endphp
                                <option value="{{ $suc->id_sucursal }}" data-icon="{{ $iconDev }}" {{ (string)$dropoffSucursalId===(string)$suc->id_sucursal ? 'selected' : '' }}>
                                    {{ $suc->nombre }}
                                </option>
                            @endforeach
                        </optgroup>
